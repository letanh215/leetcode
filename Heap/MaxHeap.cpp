...
class MaxHeap {
  ...
  void sinkDown(int index) {
    ...
    int i = 0;
    int max_index;
    int max_value = heap_[i];
    while (true) {
      max_value = max(heap_[_leftChild(i)], heap_[_rightChild(i)]);
      max_index = (max_value == heap_[_leftChild(i)]) ? _leftChild(i) : _rightChild(i);
      if (max_index < heap_.size() && max_index != index) {
        _swap(max_index, i);
        i = max_index;
      } else {
        break;
